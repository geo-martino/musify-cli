...
from mytunes_cli._io.yaml import MultiFileLoader

include_key = MultiFileLoader._include_key
...
class TestMultiFileLoader:
    ...
    def test_includes_mapping(self, data: dict[str, Any], tmp_path: Path):
        ...
        data_parent["child1"] = {include_key: path_child1.name}
        data_parent["child2"] = {include_key: str(path_child2), "other_key": "other_value"}
        with path_parent.open("w") as file:
            yaml.dump(data_parent, file)

        expected = deepcopy(data)
        expected["child1"] = data | {k: v for k, v in data_parent["child1"].items() if k != include_key}
        expected["child2"] = data | {k: v for k, v in data_parent["child2"].items() if k != include_key}
    ...
    def test_fails_on_list_include(self, data: dict[str, Any], tmp_path: Path):
        ...
        data_parent = deepcopy(data)
        data_parent["child"] = {include_key: str(path_child)}
        with path_parent.open("w") as file:
            yaml.dump(data_parent, file)
    ...
    def test_fails_on_str_include(self, data: dict[str, Any], tmp_path: Path):
        ...
        data_parent = deepcopy(data)
        data_parent["child1"] = {include_key: str(path_child)}
        with path_parent.open("w") as file:
            yaml.dump(data_parent, file)
